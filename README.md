...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create bryllup
  fly apps create bryllup-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bryllup
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bryllup-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'bryllup-staging' does not match app name in config file 'bryllup'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name bryllup-db
  fly postgres attach --app bryllup bryllup-db

  fly postgres create --name bryllup-staging-db
  fly postgres attach --app bryllup-staging bryllup-staging-db
  ```
...
trigger a deployment to your production environment,, and every commit to your `dev` branch will trigger a deployment to your staging environment.
